			...
			<div class="py-container">
				<div class="shortcut">
						...
						@if(!empty(session('user')))
							<ul class="fl">
								<li class="f-item">品优购欢迎<a style="color: red;">{{session('user.user_name')}}</a>！<a href="/quit" >退出</a></li>
							</ul>
						@else
							<ul class="fl">
								<li class="f-item">请<a href="/login" target="_blank">登录</a>　<span><a href="Index/register" target="_blank">免费注册</a></span></li>
							</ul>
						@endif
						...
						<div class="summary-wrap">
							...
							<div class="fl">
								<ul class="btn-choose unstyled">
									<li>
										<a href="javascript:;" id='button'  class="sui-btn  btn-danger addshopcar"  >加入购物车</a>
										<a href="javascript:;" id='fav'  class="sui-btn  btn-danger addshopcar"  ><?php if(in_array($goods_Info['goods_id'],$gid)){echo '取消收藏';}else{echo '收藏商品';} ?></a>
									</li>
								</ul>
							</div>
						</div>
    ...
    <input type="checkbox"><span>99</span>
					...
					<div class="tab-main intro">
						...
						<div class="tab-content tab-wraped">
							...
							@foreach($randInfo as $val)
								<div>
									<ul>
										<li>{{$val->user_name}}</li>
										<li>{{$val->rand_title}}</li>
										<li>{{date('Y-m-d H:i:s',$val->add_time)}}</li>
									</ul>
								</div>
									<?php echo '<hr>';?>
							@endforeach
								<div>
									<form action="">
										<textarea name="rand_name" id="" cols="30" rows="10"></textarea>
										<input type="button" id='rand' value='发表'>
									</form>
						...
						<div class="tbar-panel-content J-panel-content">
							<div id="J-cart-tips" class="tbar-tipbox hide">
								<div class="tip-inner">
									...
									<a href="Index/login" class="tip-btn J-login">登录</a>
								</div>
							</div>
...
$(function(){
	...
	$('#button').click(function(){
		...
		$.ajax({
			url:"/add?id={{$goods_Info['goods_id']}}",
			type:'get',
			dataType:'json',
	...
	$(document).on('click','#fav',function(){
		...
		if(val=='收藏商品'){
			...
			$.ajax({
				url:'/fav?id='+"{{$goods_Info['goods_id']}}",
				type:'get',
				dataType:'json',
				success:function(res){
					if(res.error==400001){
			...
			$.ajax({
				url:'/favtwo?id='+"{{$goods_Info['goods_id']}}",
				type:'get',
				dataType:'json',
				success:function(res){
					if(res.error==0){
	...
	$(document).on('click','#rand',function(){
		var _this = $(this)
		var val = _this.prev().val()			//获取内容
		$.ajax({
			url:'/rand?goods_id='+"{{$goods_Info['goods_id']}}",
			type:'get',
			dataType:'json',
			data:{val:val},
			success:function(res){
